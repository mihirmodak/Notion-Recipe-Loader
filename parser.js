...
async function getRecipeData(recipe_url) {
    const recipe_data_query = {
        ...
        headers: {
            "X-RapidAPI-Key":
                "00f6565afbmsh69bb7832c73481dp14a245jsn43aebbf3e65d",
            "X-RapidAPI-Host":
    ...
    try {
        ...
        console.log(
            `getRecipeData: HTTP GET --> ${recipe_response.status} ${
                recipe_response.status === 200 ? "Done" : "Failed"
            }`
        );
        return recipe_response;
    } catch (error) {
        console.error(
            "Error making the HTTP GET request for the recipe:\n",
            error
        );
        return;
    }
...
async function analyzeRecipeData(response_data) {
    ...
    try {
        ...
        console.log(
            `analyzeRecipeData: HTTP POST --> ${analyzed_response.status} ${
                analyzed_response.status === 200 ? "Done" : "Failed"
            }`
        );
        return analyzed_response;
    } catch (error) {
        console.error(
            "Error making the HTTP POST request for the analysis:\n",
            error
        );
        return;
    }
...
async function searchForIngredient(ingredient_name) {
    ...
    try {
        ...
        console.log(
            `searchForIngredient: HTTP POST --> ${search_response.status} ${
                search_response.status === 200 ? "Done" : "Failed"
            }`
        );
    } catch (error) {
        console.error(
            `Error searching for ingredient ${ingredient_name}:\n`,
            error
        );
    }
    ...
    for (const found_page of search_response.data.results) {
        if (
            found_page.properties["Item Name"].title[0].text.content ===
            ingredient_name
        ) {
...
async function createNewIngredient(
    ...
    for (const token of ingredient_category.split(";")) {
        categories.push({
            name: titleCase(cleanString(token)),
            color: "default",
        });
    }
    ...
    try {
        const create_page_response = await axios.request(
            create_ingredient_query
        );
        console.log(
            `createNewIngredient: HTTP POST --> ${
                create_page_response.status
            } ${create_page_response.status === 200 ? "Done" : "Failed"}`
        );
        return create_page_response.data.id;
    } catch (error) {
        console.error(
            `Error creating new ingredient ${ingredient_name}:\n`,
            error
        );
    }
}
async function getIngredientID(ingredient) {
    ...
    if (ingredient_id === undefined) {
        ingredient_id = await createNewIngredient(
            ingredient_name,
            ingredient_category
        );
    }
...
async function addNotionPageProperties(
    notion_page_data,
    recipe_data,
    analyzed_data
) {
    ...
    if (recipe_data.dishTypes.length > 0) {
        for (const element of recipe_data.dishTypes) {
            notion_page_data.properties.Course.multi_select.push({
                name: titleCase(cleanString(element)),
                color: "default",
            });
        }
    ...
    if (recipe_data.preparationMinutes !== -1) {
        notion_page_data.properties["Prep Time (min)"].number =
            recipe_data.preparationMinutes;
    } else {
        notion_page_data.properties["Prep Time (min)"].number =
            analyzed_data.preparationMinutes;
    }
    if (recipe_data.cookingMinutes !== -1) {
        notion_page_data.properties["Cooking Time (min)"].number =
            recipe_data.cookingMinutes;
    } else {
        notion_page_data.properties["Cooking Time (min)"].number =
            analyzed_data.cookingMinutes;
    }
    if (recipe_data.readyInMinutes !== -1) {
        notion_page_data.properties["Total Time (min)"].number =
            recipe_data.readyInMinutes;
    } else {
        notion_page_data.properties["Total Time (min)"].number =
            analyzed_data.readyInMinutes;
    }
    ...
    if (recipe_data.vegetarian && analyzed_data.vegetarian) {
        notion_page_data.properties.Tags.multi_select.push({
            name: "Vegetarian",
            color: "default",
        });
    }
    ...
    if (recipe_data.extendedIngredients.length > 0) {
        for (const ingredient of recipe_data.extendedIngredients) {
            notion_page_data.properties.Ingredients.relation.push({
                id: await getIngredientID(ingredient),
            });
        }
    }
...
async function pushNotionPage(recipe_data, analyzed_data) {
    ...
    notion_page_data = await addNotionPageProperties(
        notion_page_data,
        recipe_data,
        analyzed_data
    );
    ...
    try {
        ...
        console.log(
            `pushNotionPage: HTTP POST --> ${notion_post_response.status} ${
                notion_post_response.status === 200 ? "Done" : "Failed"
            }`
        );
...
export async function parse(url) {
    ...
    const new_notion_recipe_page = await pushNotionPage(
        recipe_response.data,
        analysis_response.data
    );
    writeResponseDataToJSON(new_notion_recipe_page, "./outputs/notion.json");

    console.log(
        `\nNew Page URL:\nhttps://www.notion.so/mihirmodak/${new_notion_recipe_page.data.properties.Name.title[0].text.content
            .split(" ")
            .join("-")}-${new_notion_recipe_page.data.id.split("-").join("")}`
    );
}
